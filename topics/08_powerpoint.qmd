...
1. **SVG** (modern PowerPoint) - good
2. **EMF** (Windows only) - obsolete and of no benifit in newer PowerPoint
3. **PDF** (cross-platform) - very bad! low res import and no editing
...
```{r}
#| label: ppt-presentation-ready
#| echo: true
#| eval: false

# Presentation version: large text, bold elements
p_present <- ggplot(mtcars, aes(wt, mpg, color = factor(cyl))) +
  ...
  labs(title = "Fuel Efficiency", color = "Cylinders")
...
p3 <- p2 + annotate("text", x = 3.5, y = 25, label = "Key finding!")
