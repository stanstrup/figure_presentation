...
p <- ggplot(curve_data, aes(x, y)) +
  ...
  annotate("text", x = 6, y = 0.5,
           label = "VECTOR\nInfinite Resolution",
           size = 8, fontface = "bold", color = "navy") +
  theme_classic(base_size = 14) +
  labs(title = "Vector Format (PDF/SVG)",
       x = "x", y = "sin(x) × exp(-x/10)")

# Save as vector
ggsave("demo_vector_curve.pdf", p, width = 6, height = 4, device = cairo_pdf)

# Save as low-res raster
ggsave("demo_lowres_curve.png", p, width = 6, height = 4, dpi = 50)

# Display side-by-side comparison
p1 <- p + labs(title = "Vector (PDF) - Smooth at any zoom")
p2 <- p +
  labs(title = "Low-res Raster (50 DPI) - Pixelated when zoomed") +
  annotate("text", x = 6, y = 0.5,
           label = "RASTER\nFixed Resolution",
...
**Try zooming the generated files:**
- `demo_vector_curve.pdf` - Stays crisp at 1000% zoom
- `demo_lowres_curve.png` - Shows pixels when zoomed
...
# Same curve, saved at different resolutions
x <- seq(0, 4*pi, length.out = 200)
raster_data <- data.frame(x = x, y = sin(x) * exp(-x/10))

base_plot <- ggplot(raster_data, aes(x, y)) +
  geom_line(color = "steelblue", linewidth = 2) +
  geom_point(data = raster_data[seq(1, 200, 20), ],
             color = "darkred", size = 3) +
  theme_classic(base_size = 14) +
  labs(x = "x", y = "sin(x) × exp(-x/10)")

# High-res raster (300 DPI)
p_highres <- base_plot +
  labs(title = "High-res PNG (300 DPI)",
       subtitle = "Looks good - 1800 × 1200 pixels") +
  annotate("text", x = 6, y = 0.5,
           label = "300 DPI\nLooks Good",
           size = 7, fontface = "bold", color = "darkgreen")

# Low-res raster (72 DPI)
p_lowres <- base_plot +
  labs(title = "Low-res PNG (72 DPI)",
       subtitle = "Pixelated when zoomed - 432 × 288 pixels") +
  annotate("text", x = 6, y = 0.5,
           label = "72 DPI\nPixelated",
           size = 7, fontface = "bold", color = "darkred")

# Save examples
ggsave("raster_highres.png", p_highres, width = 6, height = 4, dpi = 300)
ggsave("raster_lowres.png", p_lowres, width = 6, height = 4, dpi = 72)

p_highres + p_lowres
```

**Key difference:**
- High DPI = More pixels = Smoother appearance
- Low DPI = Fewer pixels = Visible pixelation
- **Both have fixed resolution** - cannot be scaled up without quality loss

::: {.callout-warning}
## Resolution Dependent
Raster images have **fixed resolution**. A 72 DPI image has only 72 pixels per inch. Zoom too much and you see individual pixels (pixelation).
:::
