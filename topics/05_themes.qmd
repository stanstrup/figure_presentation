...
# Specify which pairs to compare
my_comparisons <- list(
  c("0.5", "1"),
  c("1", "2"),
  c("0.5", "2")
...
# Alternative: Use Tukey's HSD for all pairwise comparisons
# p + stat_compare_means(method = "tukey_hsd")
```
:::
...
- Define specific pairwise comparisons
- `method = "t.test"` for pairwise tests
- `p.adjust.method = "holm"` for multiple testing correction
- `method = "anova"` for overall test
- Automatic significance brackets

**Correction methods:** "holm", "BH", "bonferroni", "hochberg", "BY", "fdr"

**Alternative:** `method = "tukey_hsd"` for Tukey's HSD (all pairwise)
:::
:::
