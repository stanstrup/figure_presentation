...
p5 <- base_plot + theme_wsj() + scale_color_wsj() +
...
::: {.callout-note}
## Install Package to See Examples

To see these themes in action, install the package:
```r
install.packages("ggthemes")
```

Then change `eval: false` to `eval: true` in the code chunk above.
:::
...
# hrbrthemes - modern professional typography
...
# Now all plots use theme_bw automatically
...
before + after +
  plot_annotation(
    title = "Theme Transformation: Default → Publication Ready",
    theme = theme(plot.title = element_text(size = 16, face = "bold"))
  )
```
