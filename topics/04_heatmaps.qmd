...
Result: Everything looks the same color! 😱
:::
...
data[5, 5] <- 20  # One extreme outlier

# Default heatmap - terrible!
library(pheatmap)
pheatmap(data)  # All data compressed to narrow range
```
...
library(massageR)  # or source the function
...
data_capped <- pmin(pmax(data, mean_val - 2*sd_val),
                    mean_val + 2*sd_val)

pheatmap(data_capped,
         main = "Values capped at ±2 SD")
```

Most common: ±2 or ±3 SD cutoff
...
lower <- quantile(data, 0.05, na.rm = TRUE)
upper <- quantile(data, 0.95, na.rm = TRUE)
...
# Log transform data
data_log <- log10(data + 1)  # +1 to handle zeros
...
4. **Always annotate** what you did: "Values capped at ±2 SD"
...
# Before: with outlier
p1 <- pheatmap(data, main = "Uncapped: Pattern lost", silent = TRUE)

# After: outlier capped
p2 <- pheatmap(data_capped, main = "Capped at ±2 SD", silent = TRUE)
