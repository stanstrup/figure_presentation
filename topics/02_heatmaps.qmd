...
Result: All the colors look pale and differences are obscured 😱
:::
...
pheatmap(expr_capped,
         main = "Values capped at ±3 SD",
         color = colorRampPalette(rev(brewer.pal(11, "RdBu")))(100),
         breaks = seq(-3, 3, length.out = 101))
```


- Scale: -3 to +3 (capped at ±3 SD)
- ~99.7% of data: -3 to +3
...
**The Outlier Correlation Problem**

Extreme values in both variables artificially inflate correlation!
...
# Create uncorrelated data with outliers
set.seed(42)
# Core data: NO correlation
demo_data <- data.frame(
  var1 = rnorm(48, mean = 0, sd = 1),
  var2 = rnorm(48, mean = 0, sd = 1)
)

# Add 2 extreme outliers in BOTH variables (same samples)
demo_data <- rbind(demo_data,
                   data.frame(var1 = c(5, -5), var2 = c(5, -5)))

# Without scaling: outliers dominate, creating spurious correlation!
p1 <- ggplot(demo_data, aes(var1, var2)) +
  geom_point(size = 2) +
  geom_point(data = demo_data[49:50, ], aes(var1, var2),
             color = "red", size = 4) +
  labs(title = "Unscaled: Outliers create fake correlation!",
       subtitle = paste("Correlation:", round(cor(demo_data$var1, demo_data$var2), 2))) +
  theme_classic()

# After scaling: true (lack of) correlation revealed
demo_scaled <- as.data.frame(scale(demo_data))
p2 <- ggplot(demo_scaled, aes(var1, var2)) +
  geom_point(size = 2) +
  geom_point(data = demo_scaled[49:50, ], aes(var1, var2),
             color = "red", size = 4) +
  labs(title = "Scaled: True relationship visible",
...
::: {.fragment}
**Key point**: Outliers (red points) pull correlation from ~0 to high value!

After scaling, outliers have less leverage → true correlation revealed.
:::
