...
set.seed(123)
# Create base metabolite levels
base_levels <- rlnorm(10, meanlog = 2, sdlog = 0.5)

# Create two groups of samples: Control and Treatment
# Control samples: close to base levels
# Treatment samples: some metabolites elevated
control_samples <- sapply(1:5, function(i) base_levels * rlnorm(10, meanlog = 0, sdlog = 0.3))
treatment_samples <- sapply(1:5, function(i) {
  effect <- ifelse(1:10 <= 5, 1.8, 1)  # First 5 metabolites elevated in treatment
  base_levels * effect * rlnorm(10, meanlog = 0, sdlog = 0.3)
})

expr_data <- cbind(control_samples, treatment_samples) %>%
