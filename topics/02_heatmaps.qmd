...
# Step 3: Cap at ±3 (only meaningful after scaling!)
...
border_color <- matrix("", nrow = nrow(capped_cells), ncol = ncol(capped_cells))
border_color[capped_cells] <- "black"

p <- pheatmap(expr_capped %>% column_to_rownames("Sample"),
              main = "Scaled + capped at ±3 (black borders = capped)",
...
# Mark capped values
border_color_q <- matrix("", nrow = nrow(capped_cells_q), ncol = ncol(capped_cells_q))
...
# Step 1: Scale to unit variance (per metabolite/column)
...
# Step 1: Scale per metabolite (column)
data_scaled <- expr_data %>%
  mutate(across(-Sample, ~ scale(.x)[,1]))

# Step 2: Identify capped values
data_mat_scaled <- data_scaled %>% column_to_rownames("Sample") %>% as.matrix()
capped_cells_prop <- (data_mat_scaled < -2.5) | (data_mat_scaled > 2.5)

# Step 3: Cap extremes at ±2.5
data_capped <- data_scaled %>%
  mutate(across(-Sample, ~ pmin(pmax(.x, -2.5), 2.5)))

# Step 4: Calculate dendrograms on scaled data
data_mat <- data_capped %>% column_to_rownames("Sample") %>% as.matrix()
row_dend <- hclust(dist(data_mat))
col_dend <- hclust(dist(t(data_mat)))

# Mark capped values
border_color_prop <- matrix("", nrow = nrow(capped_cells_prop), ncol = ncol(capped_cells_prop))
border_color_prop[capped_cells_prop] <- "black"

# Step 5: Plot with consistent scaling
p <- pheatmap(data_mat,
              cluster_rows = row_dend,
              cluster_cols = col_dend,
              scale = "none",
              ...
              border_color = border_color_prop,
...
border_color_comp <- matrix("", nrow = nrow(capped_cells_comp), ncol = ncol(capped_cells_comp))
