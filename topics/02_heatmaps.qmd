...
# First run pheatmap to get clustering order
p_temp <- pheatmap(expr_capped %>% column_to_rownames("Sample"),
                   color = colorRampPalette(rev(brewer.pal(11, "RdBu")))(100),
                   breaks = breaks, scale = "none", silent = TRUE)

# Reorder border_color matrix to match pheatmap clustering
border_color <- matrix(NA, nrow = nrow(capped_cells), ncol = ncol(capped_cells))
border_color[capped_cells] <- "black"
border_color <- border_color[p_temp$tree_row$order, p_temp$tree_col$order]

# Now create final plot with borders
p <- pheatmap(expr_capped %>% column_to_rownames("Sample"),
              main = "MAD-scaled + capped at ±3 (black borders = capped)",
              color = colorRampPalette(rev(brewer.pal(11, "RdBu")))(100),
              breaks = breaks, scale = "none",
              border_color = border_color,
              cellwidth = 15, cellheight = 15,  # Make cells larger for visible borders
              silent = TRUE)
```
...
# First run pheatmap to get clustering order
p_temp_q <- pheatmap(expr_quantile %>% column_to_rownames("Sample"),
...
# First run to get clustering order
p2_temp <- pheatmap(expr_capped_comp %>% column_to_rownames("Sample"),
                    color = colorRampPalette(rev(brewer.pal(11, "RdBu")))(100),
                    breaks = breaks_comp, scale = "none", silent = TRUE)

# Reorder border_color matrix
...
border_color_comp <- border_color_comp[p2_temp$tree_row$order, p2_temp$tree_col$order]

# Final plot with borders
