...
# Scatter plot: two samples against each other
par(mfrow = c(1, 2))

# Unscaled: correlation driven by high-variance metabolites
plot(expr_mat["Control1",], expr_mat["Control2",],
     main = "Unscaled samples",
     xlab = "Control1", ylab = "Control2")

# Scaled: true relationship visible
plot(expr_scaled["Control1",], expr_scaled["Control2",],
     main = "Scaled samples",
     xlab = "Control1 (scaled)", ylab = "Control2 (scaled)")
```
...
4. **Scale data BEFORE** passing to heatmap function
5. **Calculate dendrograms** on the same scaled data
...
4. **Use appropriate palette**:
   - Diverging for data with meaningful center (red-white-blue)
   - Sequential for one-directional (viridis, magma)
:::
