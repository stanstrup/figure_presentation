...
group2 <- matrix(rlnorm(50, meanlog = 4, sdlog = 0.8), nrow = 5, ncol = 10)
...
for (i in 1:5) {  # First 5 metabolites get outliers
  ...
  normal_range <- range(expr_data[[metab_col]])
  range_size <- diff(normal_range)
  ...
  for (s in outlier_samples) {
    ...
    if (runif(1) > 0.5) {
      expr_data[s, metab_col] <- normal_range[2] * runif(1, 2, 4)
    } else {
      expr_data[s, metab_col] <- normal_range[1] * runif(1, 0.25, 0.5)
    }
  }
}
...
expr_log <- expr_data %>%
  mutate(across(-Sample, ~ log2(.x)))

# Use divergent ColorBrewer scale centered at mean
# RdBu = Red-Blue divergent scale (reversed so blue is high)
expr_mat_outlier <- expr_log %>%
  column_to_rownames("Sample") %>%
  as.matrix()

data_center <- mean(expr_mat_outlier, na.rm = TRUE)
max_diff <- max(abs(expr_mat_outlier - data_center), na.rm = TRUE)
...
# Cap at 5th and 95th percentiles PER COLUMN (metabolite)
...
p <- pheatmap(expr_quantile %>% column_to_rownames("Sample"),
              main = "Values capped at 5-95 percentiles (per metabolite)",
              color = colorRampPalette(rev(brewer.pal(11, "RdBu")))(100),
              silent = TRUE)
```
...
- Uses percentiles **per metabolite**
- Handles different scales across metabolites
...
# Create positive data (e.g., count data for metabolites)
set.seed(123)
count_data <- matrix(rpois(100, lambda = 50), nrow = 10, ncol = 10) %>%
  as_tibble(.name_repair = ~paste0("Metabolite_", 1:10)) %>%
  mutate(Sample = paste0("Sample", 1:10), .before = 1)

count_data[5, "Sample5"] <- 5000  # Extreme count

# Log transform BEFORE plotting
count_log <- count_data %>%
  mutate(across(-Sample, ~ log10(.x + 1)))  # +1 to handle zeros

p <- pheatmap(count_log %>% column_to_rownames("Sample"),
