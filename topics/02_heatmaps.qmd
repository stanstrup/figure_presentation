...
expr_mat_outlier <- expr_data %>% column_to_rownames("Gene")
max_abs <- max(abs(range(expr_mat_outlier)))
...
# Create symmetric breaks centered at 0
expr_mat <- expr_capped %>% column_to_rownames("Gene")
max_abs <- max(abs(range(expr_mat)))
breaks <- seq(-max_abs, max_abs, length.out = 101)
