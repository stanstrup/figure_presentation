...
```{r}
#| label: interactive-ggplotly-basic
#| echo: true
#| eval: false
...
```{r}
#| label: interactive-plotly-basic
#| echo: true
#| eval: false
...
```{r}
#| label: interactive-timeseries
#| echo: true
#| eval: false

# Stock price example (using built-in data)
ts_data <- data.frame(date = seq(as.Date("2023-01-01"), length.out = 252, by = "day"), price = cumsum(rnorm(252)))
plot_ly(ts_data, x = ~date, y = ~price, type = 'scatter', mode = 'lines') %>%
  layout(
    xaxis = list(
      rangeselector = list(
        buttons = list(
...
```{r}
#| label: interactive-filesize
#| echo: true
#| eval: false

# For large datasets, sample for interactive version
library(dplyr)
mtcars_sample <- mtcars %>% slice_sample(n = nrow(mtcars))  # mtcars is small, so no real sampling needed

p <- ggplot(mtcars_sample, aes(wt, mpg)) + geom_point()
ggplotly(p)
...
```{r}
#| label: interactive-complete-example
#| echo: true
#| eval: false
