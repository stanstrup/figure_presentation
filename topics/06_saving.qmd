...
ggplot(iris_setosa, aes(x = Sepal.Length, y = Sepal.Width)) +
  ...
  stat_regline_equation(
    aes(label = paste(after_stat(eq.label), after_stat(p.label), sep = "~~~~")),
    formula = y ~ x
  ) +
  stat_cor(
...
plot_data <- iris %>%
  nest(data = -Species) %>%
  mutate(plot = map2(data, as.character(Species), make_species_plot))
```
...
4. Save with `walk2()` using plot and Species columns
