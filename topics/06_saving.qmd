...
make_plot <- function(data, title) {
  ...
  } else if (title == "faithful") {
    ggplot(data, aes(eruptions)) +
      geom_histogram(bins = 30) +
      labs(title = "Eruption duration")
  }
}

# Create tibble and apply plotting
plot_data <- tibble(
  title = c("mtcars", "iris", "faithful"),
  data = list(mtcars, iris, faithful)
) %>%
  mutate(plot = map2(data, title, make_plot))

# Save all plots using iwalk
iwalk(plot_data$plot, ~ggsave(
  filename = glue("{fig_dir}/figure_{plot_data$title[.y]}.pdf"),
  plot = .x,
  width = 7, height = 5
))
```
...
2. Create tibble with data
3. Use `map2()` to apply function
4. Save with `iwalk()`

**Result:** Organized data structure with all plots
:::
:::
...
# A tibble: 3 × 3
  title    data               plot
  <chr>    <list>             <list>
1 mtcars   <df [32 × 11]>     <gg>
2 iris     <df [150 × 5]>     <gg>
3 faithful <df [272 × 2]>     <gg>
```
...
**Example output:** One of the generated plots
:::
:::
