...
```r
# Small dimensions = larger text relative to plot
ggsave("figure.pdf", width = 3.5, height = 3)

# Large dimensions = smaller text relative to plot
ggsave("figure.pdf", width = 10, height = 8)
```

Then use **DPI** (or vector formats) to get the resolution you need.
:::
...
1. **Set dimensions to match final output size**
   - Journal single column: ~3.5 inches
...
3. **Use appropriate format**
   - Vector (PDF) for publications - infinite resolution
...
ggsave("small.pdf", p, width = 3.5, height = 3)   # Small canvas
ggsave("large.pdf", p, width = 7, height = 6)     # Large canvas

# The difference: text appears larger in small.pdf
```
:::
...
**Vector formats (PDF, SVG) ignore DPI!**
...
ggsave("plot.png", p, width = 5, height = 4, dpi = 300)   # 1500 × 1200 pixels
ggsave("plot.png", p, width = 5, height = 4, dpi = 72)    # 360 × 288 pixels
```
...
ggsave("figure1.pdf", p, width = 3.5, height = 3, device = cairo_pdf)

# STEP 2: If text still too small/large, adjust base_size
p2 <- p + theme_classic(base_size = 10)  # Slightly smaller base
ggsave("figure1_v2.pdf", p2, width = 3.5, height = 3, device = cairo_pdf)
```
...
ggsave("paper.pdf", p + theme_classic(base_size = 11),
       width = 3.5, height = 3)

# For presentation (large, readable from distance)
ggsave("slides.png", p + theme_classic(base_size = 18),
...
5. **Vector formats** for publications (PDF, SVG)
