...
ggplot(correct_data, aes(x = x, y = y)) +
  ...
  geom_text(aes(label = label), size = 8, fontface = "bold", color = "white") +
  scale_fill_identity() +
  scale_x_continuous(breaks = 1:k, labels = anchors_reversed, position = "bottom") +
  theme_void() +
  coord_fixed(ratio = 0.2) +
  theme(
  ...
  labs(title = "Correct Rainbow Order",
       subtitle = paste("Colors:", paste(anchors_reversed, collapse = " → ")))
```
...
jet    <- data.frame(x = n:1, y = 1, color = rev(mixOmics::color.jet(n)))
